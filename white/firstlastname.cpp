...
public:
  Person(const string& f, const string& l, int y) {
    birthday = y;
    first_name_history.push_back({ y, f });
    last_name_history.push_back({ y, l });
  };
...
string Person::GetName(int year, const vector<pair<int, string>>& history) const {
  const auto& name_pair = find_if(begin(history), end(history), [year](const auto& name_pair) {
    return name_pair.first <= year;
  });

  if (name_pair == end(history)) {
    return "";
  }
