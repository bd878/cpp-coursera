...
public:
  ...
  ReadAccess At(const K& key) const {
    const MapType& bucket = data.at(hasher(key) % buckets);
  ...
  WriteAccess operator[](const K& key) {
    ...
    mutex& bucket_m = locks[hasher(key) % buckets];
    MapType& bucket = data[hasher(key) % buckets];
    return {lock_guard(bucket_m), bucket[key]};
  }
  ...
  MapType BuildOrdinaryMap() const {
    ...
    for (size_t i = 0; i < data.size(); ++i) {
      lock_guard<mutex> bucket_guard(locks[i]);
      const MapType& bucket = data[i];
