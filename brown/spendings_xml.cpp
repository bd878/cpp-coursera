...
vector<Spending> SolutionLoadFromXml(istream& input) {
  Document xml_doc = Load(input);
  vector<Spending> result;
  for (const auto& spending : xml_doc.GetRoot().Children()) {
    result.push_back({
      spending.AttributeValue<string>("category"),
      spending.AttributeValue<int>("amount")
    });
  }
  return result;
}
...
vector<Spending> LoadFromXml(istream& input) {
  ...
  const vector<Node>& xml_spendings = xml_doc.GetRoot().Children();
  vector<Spending> result;
  result.reserve(xml_spendings.size());
  transform(xml_spendings.begin(), xml_spendings.end(), back_inserter(result), [](const Node& xml_spending)->Spending{
    return {
      xml_spending.AttributeValue<string>("category"),
      xml_spending.AttributeValue<int>("amount")
    };
  });
