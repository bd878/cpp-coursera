...
struct AddressHasher {
  ...
  size_t operator()(const Address& addr) const {
    ...
    return (
      coef * coef * string_hasher(addr.city) +
      coef * string_hasher(addr.street) +
      int_hasher(addr.building)
    );
  }
};
...
struct PersonHasher {
  ...
  size_t operator()(const Person& p) const {
    ...
    return (
      coef * coef * coef * string_hasher(p.name) +
      coef * coef * int_hasher(p.height) +
      coef * double_hasher(p.weight) +
      addr_hasher(p.address)
    );
  }
};
